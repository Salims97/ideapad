...
function init() {
  ...
  // scene.add(new THREE.AxesHelper(0.079));
...
//control the camera movement
  ...
  // groupRocket.rotateZ( - angleOfAttack);
  mdot = 1250;
  rocketMass = 33000;
  ...
  control = new function () {
    this.thrust = 3800000;
    this.fuelMass = 330000;
    this.rho = 1.3;
    this.liftCoefficient = 0.75;
    this.dragCoefficient = 0.75;
  };
  addControls(control);
  function addControls(controlObject) {
    var gui = new GUI();
    gui.add(controlObject, 'thrust', 0, 4200000);
    gui.add(controlObject, 'fuelMass', 0, 50000);
...
function updatePhysics() {
  ...
  drag = 0.5 * control.rho * control.liftCoefficient * referenceArea * velocity.length() * velocity.length();
  fDrag = new THREE.Vector3(-drag * Math.cos(angleOfAttack), -drag * Math.sin(angleOfAttack), 0);

  lift = 0.5 * control.rho * control.dragCoefficient * referenceArea * velocity.length() * velocity.length();
  fLift = new THREE.Vector3(-lift * Math.sin(angleOfAttack), -lift * Math.cos(angleOfAttack), 0);
  ...
  camera1.position.set(groupRocket.position.x, groupRocket.position.y + 0.01, groupRocket.position.z + 0.2);
 


  // centerOfEarth.copy(groupRocket.position);
  // centerOfEarth.normalize();
 
  

  // rocketPosition.applyAxisAngle(groupRocket , 1.2);
}
...
setInterval(function () {
  ...
  console.log(thetaRocket);
  // console.log(thetaRocket, theta);
  ...
  // rocketPosition.add(centerOfEarth);
  // rotateAngle = centerOfEarth.angleTo(groupRocket.position);

  // console.log(centerOfEarth,rocketPosition,rotateAngle);
  // if (rotateAngle < 1.5707) {
    // groupRocket.rotation.z =   -thetaRocket;
